...
import MenuService from "../../services/menu-api.tsx";
...
const AddMenuItem = ({ onSubmit, onCancel, initialItem }: AddMenuItemProps) => {
  ...
  const handleImageChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files[0]) {
        ...
        try {
          const data = await MenuService.uploadMenuImage(file);
  ...
  const addModifier = () => {
    setModifiers([
      ...modifiers,
      { name: "", min_choices: 0, max_choices: 1, choices: [] },
    ]);
  };
  ...
  return (
    <div>
      <div className="add-menu-item-header">
        <h3 className="add-menu-item-title">
      ...
      <form onSubmit={handleSubmit} className="add-menu-item-form">
        ...
        <div className="add-menu-item-form-group">
          <label htmlFor="image" className="add-menu-item-label">
            Image
          </label>

          <div className="add-menu-item-image-upload">
            <input
              type="file"
              id="image"
              accept="image/*"
              onChange={handleImageChange}
              className="add-menu-item-image-input"
              disabled={isUploading}
            />
            <label htmlFor="image" className="add-menu-item-image-label">
              <Upload size={24} />
              <span>
                {isUploading
                  ? "Uploading..."
                ...
                <div className="add-menu-item-number-input">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateModifier(modifierIndex, {
                        ...modifier,
                        min_choices: Number(e.target.value),
                      })
                    }
                    min="0"
                    className="add-menu-item-input"
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const updatedModifier = {
                        ...modifier,
                        max_choices: newMaxChoices,
                      };
                      if (newMaxChoices < modifier.choices.length) {
                        updatedModifier.choices = modifier.choices.slice(
                          0,
                          newMaxChoices,
                        );
                      }
                ...
                {modifier.choices.map((choice, choiceIndex) => (
                  <div key={choiceIndex} className="add-menu-item-choice">
                    <input
                      ...
                      value={choice.name}
                      onChange={(e) =>
                        updateChoice(modifierIndex, choiceIndex, {
                          ...choice,
                          name: e.target.value,
                        })
                      }
        ...
        <div className="add-menu-item-button-group">
          <button
            type="submit"
            className={`${"add-menu-item-button"} ${"add-menu-item-submit-button"}`}
            disabled={isUploading}
          >
            {isUploading
              ? "Uploading..."
          ...
          <button
            type="button"
            onClick={onCancel}
            className={`${"add-menu-item-button"} ${"add-menu-item-cancel-button"}`}
          >
            Cancel
          </button>
        </div>
